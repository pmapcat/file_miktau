...
func TestPerformance(t *testing.T) {
	...
	timeEval(fmt.Sprintf("With faceting get_app_data on %v", perfsize), func() {
		assert.Equal(t, int(res.GetAppData(*newQuery().WithTags("natan", "магазины", "sforim")).TotalNodes), 13120)
	})
	timeEval(fmt.Sprintf("Just default app data gotten on %v", perfsize), func() {
		assert.Equal(t, int(res.GetAppData(*newQuery()).TotalNodes), 100001)
	})

	timeEval(fmt.Sprintf("Checking amount of nodes, because it should be less than %v %v", DEFAULT_PAGE_SIZE, perfsize), func() {
		assert.Equal(t, int(len(res.GetAppData(*newQuery().WithTags("natan", "магазины", "sforim")).Nodes)), DEFAULT_PAGE_SIZE)
	})
...
func TestAllNodesShouldHaveDifferentIdInSearchResponse(t *testing.T) {
	cnis := newAppState("testing")
	cnis.MutableCreate(buildDemoDataset())
	nodes := cnis.GetAppData(*newQuery()).Nodes
...
func TestCloudShouldntHaveEmptyTags(t *testing.T) {
	cnis := newAppState("testing")
	cnis.MutableCreate(buildDemoDataset())
	assert.Equal(t, cnis.GetAppData(*newQuery()).SimpleCloud(), map[string]int{"work": 20, "bibliostore": 8, "translator": 2, "natan": 13, "wiki": 1, "everybook": 1, "amazon": 2, "согласовать": 1, "moscow_market": 9, "sforim": 2, "скачка_источников": 1, "биржа": 2, "магазины": 2, "UI": 1, "personal": 4, "blog": 1, "devops": 1, "zeldin": 2, "usecases": 2, "работа_сделана": 1})

	for tag_name, _ := range cnis.GetAppData(*newQuery()).SimpleCloud() {
...
func TestCloudCouldSelectShouldntHaveEmptyTags(t *testing.T) {
	cnis := newAppState("testing")
	cnis.MutableCreate(buildDemoDataset())
	for tag_name, _ := range cnis.GetAppData(*newQuery()).CloudCanSelect {
		assert.NotEqual(t, tag_name, "")
		assert.NotEqual(t, strings.TrimSpace(tag_name), "")
	}
}
...
func TestGettingStationaryAppData(t *testing.T) {
	...
	assert.Equal(t, cnis.GetAppData(*newQuery().WithTags("natan")).SimpleCloudCanSelect(),
	...
	assert.Equal(t, cnis.GetAppData(*newQuery()).SimpleCloudCanSelect(),
	...
	// test sorted results
	assert.Equal(t, cnis.GetAppData(*newQuery().OrderBy("modified")).Nodes[0].Modified.Time().String(), "2018-07-19 00:00:00 +0000 UTC")
	assert.Equal(t, cnis.GetAppData(*newQuery().OrderBy("-modified")).Nodes[0].Modified.Time().String(), "2016-02-05 00:00:00 +0000 UTC")

	assert.Equal(t, cnis.GetAppData(*newQuery().OrderBy("name")).Nodes[0].Name, "blab.mp4")
	assert.Equal(t, cnis.GetAppData(*newQuery().OrderBy("-name")).Nodes[0].Name, "zlop.mp4")

}
func TestAggregation(t *testing.T) {
	cnis := newAppState("testing")
	cnis.MutableCreate(buildDemoDataset())
	assert.Equal(t, cnis.GetAppData(*newQuery()).Patriarchs, []string{"work", "работа_сделана"})
	assert.Equal(t, cnis.GetAppData(*newQuery()).TotalNodes, 22)
	assert.Equal(t, len(cnis.GetAppData(*newQuery()).CloudContext), len(cnis.GetAppData(*newQuery()).Cloud))

	assert.Equal(t, cnis.GetAppData(*newQuery()).SimpleCloudContext()["work"], map[string]int{"natan": 7, "bibliostore": 6, "moscow_market": 6, "translator": 1, "amazon": 1})
	assert.Equal(t, cnis.GetAppData(*newQuery()).SimpleCloudContext()["translator"],
...
func TestMutableWorkage(t *testing.T) {
	...
	assert.Equal(t, cnis.GetAppData(*newQuery()).SimpleCloud(), map[string]int{"devops": 1, "translator": 2, "wiki": 1,
...
func TestFixingEmptyFieldsForDataSet(t *testing.T) {
	storage := newAppState("empty")
	storage.MutableRebirthWithNewData(buildDemoDataset())

	assert.Equal(t, storage.GetAppData(*newQuery()).SimpleCloud(), map[string]int{"devops": 1, "wiki": 1, "zeldin": 2, "work": 20, "sforim": 2,
...
func TestGetAppData(t *testing.T) {
	...
	// assert.Equal(t, cnis.GetAppData(*newQueryWithFullResponse()).Cloud, "blab")

	// typical query (mixed)

	// typical query (only tags)
	assert.Equal(t, len(cnis.GetAppData(*newQuery().WithTags("natan").WithPage(2)).Nodes), 3)
	// typical query (empty request)
	assert.Equal(t, len(cnis.GetAppData(*newQuery().WithPage(3)).Nodes), 2)
}
...
func TestMetaGetAppData(t *testing.T) {
	on_time.WithWhateverTime(on_time.OnDate(2003, 02, 17), func() {
		cnis := newAppState("testing")
		cnis.MutableCreate(buildDemoDataset())
		assert.Equal(t, cnis.GetAppData(*newQuery()).MetaCloud(),
		...
		assert.Equal(t, len(cnis.GetAppData(*newQuery().WithTags("@modified-year:2018")).MetaCloudCanSelect()), 6)
		assert.Equal(t, len(cnis.GetAppData(*newQuery().WithTags("@modified-year:2016")).MetaCloudCanSelect()), 13)
		assert.Equal(t, len(cnis.GetAppData(*newQuery().WithTags("@modified-year:2017")).MetaCloudCanSelect()), 10)
	})

	cnis := newAppState("testing")
	cnis.MutableCreate(buildDemoDataset())
	// meta-cloud-context should be in ordinary taxonomies/tags
	assert.Equal(t, cnis.GetAppData(*newQuery()).MetaCloudContext()["@file-type:video"],
...
func TestDachaBaselineDataset(t *testing.T) {
	...
	assert.Equal(t, cnis.GetAppData(*newQuery().WithTags("дервья")).SimpleCloud(),
	...
	assert.Equal(t, cnis.GetAppData(*newQuery().WithTags("деревья")).SimpleCloudCanSelect(),
