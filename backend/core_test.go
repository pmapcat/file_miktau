...
func TestPerformance(t *testing.T) {
	...
	timeEval(fmt.Sprintf("Baseline get_app_data on %v", perfsize), func() {
		res.GetAppData(*newCoreQueryWithFullResponse())
	})
	timeEval(fmt.Sprintf("With sorting get_app_data on %v", perfsize), func() {
		res.GetAppData(*newCoreQueryWithFullResponse().OrderBy("-modified"))
	})
	...
	timeEval(fmt.Sprintf("With faceting get_app_data on %v", perfsize), func() {
		assert.Equal(t, int(res.GetAppData(*newCoreQueryWithFullResponse().WithTags("natan", "магазины", "sforim")).TotalNodes), 13234)
	})

	timeEval(fmt.Sprintf("Checking amount of nodes, because it should be less than %v %v", DEFAULT_PAGE_SIZE, perfsize), func() {
		assert.Equal(t, int(len(res.GetAppData(*newCoreQueryWithFullResponse().WithTags("natan", "магазины", "sforim")).Nodes)), DEFAULT_PAGE_SIZE)
	})
...
func TestAddingBug(t *testing.T) {
	...
	cnis.MutableUpdateInBulk(*newCoreQueryWithFullResponse(), func(item *CoreNodeItem) *CoreNodeItem {
		item.Tags = []string{}
		return item
	})
	// assert that everything is empty
	// assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse()).Cloud), 0)
	// assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse()).Nodes), 22)
	// assert.Equal(t, cnis.MutableAddRemoveTagsToSelection(*newCoreQueryWithFullResponse().WithFilePathes("/home/mik/this_must_be_it/hello.mp4"), []string{"zanzibar"}, []string{}), 1)
	// assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse().WithFilePathes("/home/mik/this_must_be_it/hello.mp4")).Nodes), 1)

	// assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse()).Cloud), 1)
...
func TestApplyFilter(t *testing.T) {
	...
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithTags("natan")), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithTags("natan", "work")), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithTags("natan", "work", "ZAZAZAZA")), false)

	// check empty tag mechanics
	assert.Equal(t, cnis.nodes[0].ApplyFilter(newCoreQueryWithFullResponse()), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse()), true)
	// check select by id mechanics
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithIds(1)), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithIds(0, 2)), false)

	// check choose by date mechanics
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithDate(2017, 0, 0)), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithDate(2016, 0, 0)), false)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithDate(0, 7, 0)), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithDate(0, 8, 0)), false)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithDate(0, 0, 20)), true)
	assert.Equal(t, cnis.nodes[1].ApplyFilter(newCoreQueryWithFullResponse().WithDate(0, 0, 21)), false)
...
func TestAllNodesShouldHaveDifferentIdInSearchResponse(t *testing.T) {
	cnis := newCoreNodeItemStorage("testing")
	cnis.MutableAddMany(buildDemoDataset())
	nodes := cnis.GetAppData(*newCoreQueryWithFullResponse().WithFullResponse()).Nodes
...
func TestCloudShouldntHaveEmptyTags(t *testing.T) {
	cnis := newCoreNodeItemStorage("testing")
	cnis.MutableAddMany(buildDemoDataset())
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse()).Cloud, map[string]int{"work": 20, "bibliostore": 8, "translator": 2, "natan": 13, "wiki": 1, "everybook": 1, "amazon": 2, "согласовать": 1, "moscow_market": 9, "sforim": 2, "скачка_источников": 1, "биржа": 2, "магазины": 2, "UI": 1, "personal": 4, "blog": 1, "devops": 1, "zeldin": 2, "usecases": 2, "работа_сделана": 1})

	for tag_name, _ := range cnis.GetAppData(*newCoreQueryWithFullResponse()).Cloud {
...
func TestCloudCouldSelectShouldntHaveEmptyTags(t *testing.T) {
	cnis := newCoreNodeItemStorage("testing")
	cnis.MutableAddMany(buildDemoDataset())
	for tag_name, _ := range cnis.GetAppData(*newCoreQueryWithFullResponse()).CloudCanSelect {
		assert.NotEqual(t, tag_name, "")
		assert.NotEqual(t, strings.TrimSpace(tag_name), "")
	}
}
...
func TestGettingTaggingTree(t *testing.T) {
	cnis := newCoreNodeItemStorage("testing")
	cnis.MutableAddMany(buildDemoDataset())
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().WithFullResponse()).TreeTag.Show("-"), `-root 
--work 
---everybook 
---natan 
----bibliostore 
...
func TestGettingStationaryAppData(t *testing.T) {
	...
	cnis.MutableAddMany(buildDemoDataset())
	// test getting cloud
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse()).Cloud,
	...
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse()).Calendar, CoreDateFacet{
	...
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("zeldin")).CalendarCanSelect, CoreDateFacet{
		Year:  map[int]int{2016: 2},
		Month: map[int]int{5: 2},
		Day:   map[int]int{3: 1, 2: 1}})
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse()).CalendarCanSelect, CoreDateFacet{
		Year:  map[int]int{2018: 2, 2016: 14, 2017: 6},
		Month: map[int]int{5: 4, 3: 1, 1: 1, 7: 4, 2: 8, 4: 4},
		Day:   map[int]int{4: 1, 3: 1, 14: 1, 12: 1, 10: 1, 8: 1, 5: 2, 18: 1, 13: 1, 11: 1, 2: 1, 24: 1, 21: 1, 19: 1, 17: 1, 16: 1, 9: 1, 20: 1, 15: 1, 7: 1, 1: 1}})

	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("natan")).CloudCanSelect,
	...
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse()).CloudCanSelect,
	...
	// test sorted results
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().OrderBy("modified")).Nodes[0].Modified, CoreDateField{Year: 2018, Month: 7, Day: 19})
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().OrderBy("-modified")).Nodes[0].Modified, CoreDateField{Year: 2016, Month: 2, Day: 5})

	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().OrderBy("name")).Nodes[0].Name, "blab.mp4")
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().OrderBy("-name")).Nodes[0].Name, "zlop.mp4")
...
func TestMutableWorkage(t *testing.T) {
	cnis := newCoreNodeItemStorage("testing")
	cnis.MutableAddMany(buildDemoDataset())
	cnis.MutableAddRemoveTagsToSelection(*newCoreQueryWithFullResponse().WithTags("work"), []string{"bolo"}, []string{"work"})
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse()).Cloud, map[string]int{"devops": 1, "translator": 2, "wiki": 1,
...
func TestFixingEmptyFieldsForDataSet(t *testing.T) {
	...
	assert.Equal(t, storage.GetAppData(*newCoreQueryWithFullResponse()).Cloud, map[string]int{"devops": 1, "wiki": 1, "zeldin": 2, "work": 20, "sforim": 2,
...
func TestGetAppData(t *testing.T) {
	...
	// typical query (mixed)
	assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("natan", "магазины", "sforim").WithDate(2017, 04, 10).OrderBy("-modified")).Nodes), 1)

	// typical query (only tags)
	assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("natan").WithPage(2)).Nodes), 3)
	// typical query (only date)
	assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse().WithDate(2016, 2, 5)).Nodes), 1)
	// typical query (empty request)
	assert.Equal(t, len(cnis.GetAppData(*newCoreQueryWithFullResponse().WithPage(3)).Nodes), 2)
}
...
func TestGettingNodesByFilePaths(t *testing.T) {
	...
	counter := 0
	cnis.GetInBulk(*newCoreQueryWithFullResponse().WithFilePathes("/home/mik/chosen_one/",
...
func TestDachaBaselineDataset(t *testing.T) {
	...
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("дервья")).Cloud,
	...
	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("деревья")).CloudCanSelect,
		map[string]bool{"дача": true, "сад": true, "деревья": true, "кусты": true, "цветники": true})

	assert.Equal(t, cnis.GetAppData(*newCoreQueryWithFullResponse().WithTags("деревья")).Calendar, CoreDateFacet{Year: map[int]int{2017: 37}, Month: map[int]int{2: 37}, Day: map[int]int{26: 12, 24: 25}})
