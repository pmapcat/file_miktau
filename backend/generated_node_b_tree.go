...
func (p *btEpool) get(err error, hit bool, i int, k uint32, q *d, t *Tree, ver int64) *Enumerator {
	x := p.Get().(*Enumerator)
...
func (q *x) insert(i int, k uint32, ch interface{}) *x {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
// true.
func (t *Tree) Delete(k uint32) (ok bool) {
	pi := -1
	var p *x
	q := t.r
	if q == nil {
...
func (t *Tree) extract(q *d, i int) { // (r NodeItem) {
	t.ver++
...
func (t *Tree) find(q interface{}, k uint32) (i int, ok bool) {
	var mk uint32
	l := 0
	switch x := q.(type) {
	case *x:
...
func (t *Tree) First() (k uint32, v NodeItem) {
	if q := t.first; q != nil {
		q := &q.d[0]
		k, v = q.k, q.v
	}
...
func (t *Tree) Get(k uint32) (v NodeItem, ok bool) {
	q := t.r
	if q == nil {
		return
	}
...
func (t *Tree) insert(q *d, i int, k uint32, v NodeItem) *d {
	t.ver++
	c := q.c
	if i < c {
...
// (zero-value, zero-value) if the tree is empty.
func (t *Tree) Last() (k uint32, v NodeItem) {
	if q := t.last; q != nil {
		q := &q.d[q.c-1]
		k, v = q.k, q.v
	}
...
func (t *Tree) overflow(p *x, q *d, pi, i int, k uint32, v NodeItem) {
	t.ver++
	l, r := p.siblings(pi)
...
func (t *Tree) Seek(k uint32) (e *Enumerator, ok bool) {
	q := t.r
	if q == nil {
...
func (t *Tree) Set(k uint32, v NodeItem) {
	//dbg("--- PRE Set(%v, %v)\n%s", k, v, t.dump())
	//defer func() {
	//	dbg("--- POST\n%s\n====\n", t.dump())
	//}()
...
//
// 	tree.Put(k, func(uint32, bool){ return v, true })
//
// modulo the differing return values.
func (t *Tree) Put(k uint32, upd func(oldV NodeItem, exists bool) (newV NodeItem, write bool)) (oldV NodeItem, written bool) {
	pi := -1
	var p *x
	q := t.r
	var newV NodeItem
...
func (t *Tree) split(p *x, q *d, pi, i int, k uint32, v NodeItem) {
	t.ver++
	r := btDPool.Get().(*d)
	if q.n != nil {
		r.n = q.n
...
// io.EOF is returned.
func (e *Enumerator) Next() (k uint32, v NodeItem, err error) {
...
// == io.EOF is returned.
func (e *Enumerator) Prev() (k uint32, v NodeItem, err error) {
	if err = e.err; err != nil {
		return
	}
